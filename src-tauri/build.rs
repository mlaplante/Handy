...
fn build_apple_speech_bridge() {
    ...
    // See build_apple_intelligence_bridge's SDKROOT/SWIFTC note — same
    // env-override pattern for non-Xcode toolchains.
    let sdk_path = env::var("SDKROOT").unwrap_or_else(|_| {
        String::from_utf8(
            Command::new("xcrun")
                .args(["--sdk", "macosx", "--show-sdk-path"])
    ...
    // Unlike FoundationModels (which needs an Xcode-only macro plugin, see
    // is_command_line_tools_only below), the Speech framework's
    // SpeechAnalyzer/SpeechTranscriber types compile cleanly under a plain
    // Command-Line-Tools swiftc (confirmed via a recon spike: Apple Swift 6.4
    // / SDK 27 compiled the real bridge with no Xcode installed). So the only
    // real/stub gate here is whether the active SDK ships Speech.framework at
    // all, plus an explicit escape hatch for forcing the stub.
    ...
    let source_file = if framework_path.exists() && !force_stub {
        println!("cargo:warning=Building with Apple Speech (SpeechAnalyzer) support.");
        REAL_SWIFT_FILE
    } else {
        if framework_path.exists() {
            println!("cargo:warning=Building Apple Speech with stubs (HANDY_FORCE_SPEECH_STUB=1).");
        } else {
            println!("cargo:warning=Speech.framework not found in SDK. Building Apple Speech with stubs.");
        }
        STUB_SWIFT_FILE
    };
    ...
    let status = Command::new(&swiftc_path)
        .args([
            // See build_apple_intelligence_bridge's note on -parse-as-library.
            "-parse-as-library",
            "-target",
            "arm64-apple-macosx11.0",
            "-sdk",
